...
waze_data['hour'] = waze_data['hour'].dt.strftime('%H:%M')
...
app_ui = ui.page_fluid(
    ...
    ui.input_slider("hour", "Hour of the Day", 0, 23, 20), 
...
def server(input, output, session):
    ...
    @render.plot
    def my_plot():
        # Extract the selected type and subtype
        ...
        # Convert the hour to the desired format if necessary 
        hour_str = f'{int(selected_hour):02}:00'

        # Convert the 'hour' column to numeric for comparison
        waze_data['hour_numeric'] = pd.to_datetime(waze_data['hour'], format='%H:%M').dt.hour

        # Filter the dataset for the selected type and subtype
        filtered_waze_data = waze_data[(waze_data['type'] == selected_type) & (waze_data['updated_subtype'] == selected_subtype) & (waze_data['hour'] == hour_str)]
        ...
        ax.set_title(f'Scatter Plot of {selected_type_subtype} Cases')
